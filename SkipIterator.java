...
public class SkipIterator {
    ...
    public void skip(int val) { //O(s) S.C, where s is no of elements being skipped
        if(val == nextEl) skipCheck(); //if the current global nextEl is the one to be skipped
        else {
            if(!map.containsKey(val)) map.put(val, 0); //check if the skip map already contains the input val
            map.put(val, map.get(val) + 1); //increment the value of this input in the map
        }
    }

    public void skipCheck() { //T.C -> O(1) amortized, O(n) W.C, where n is the count of elements in iterator
